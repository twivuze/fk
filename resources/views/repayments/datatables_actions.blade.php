{!! Form::open(['route' => ['repayments.destroy', $id], 'method' => 'delete']) !!}
<div class='btn-group'>
    <!-- <a href="{{ route('repayments.show', $id) }}" class='btn btn-default btn-xs'>
        <i class="glyphicon glyphicon-eye-open"></i>
    </a>
    <a href="{{ route('repayments.edit', $id) }}" class='btn btn-default btn-xs'>
    ...
    <?php $invoice= \App\Models\Repayment::where('id',$id)->first();
if($invoice){
?>
...
<?php if($invoice->status=='pending' || $invoice->status=='outstanding'){?>
    ...
    <a class="btn btn-primary btn-xs" href="/repayment/{{$invoice->repay_code}}" onclick="centeredPopup(this.href,'myWindow','900','600','yes');return false" class='btn btn-default btn-xs'>Repay</a>
<?php } ?>
    <br><br>
    <a href="#" class='btn btn-info btn-xs ' onclick="event.preventDefault(); printThis(<?php echo $id; ?>);">
        <i class="glyphicon glyphicon-eye-open"></i> View Invoice &nbsp;
    </a>
